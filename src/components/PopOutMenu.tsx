  ...
  return (
    <>
      <a
        ...
        <FontAwesomeIcon className="text-[#6b778c]" icon={["fas", "ellipsis"]} />
        {menuOpen && (
          <div className="absolute text-sm top-[100%] left-0 bg-white w-64 rounded-[3px] shadow-sm z-20">
            <header className="py-2 text-center text-[#5e6c84]">List actions</header>

            {props.items.map((item: PopoutMenuItemType, index: number) => {
              return <PopOutMenuItem key={index} onClick={item.fn} item={item} />;
            })}
          </div>
        )}
      </a>
